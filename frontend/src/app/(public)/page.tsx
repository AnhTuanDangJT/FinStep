...
export default function LandingPage() {
  // State architecture as specified
  ...
  useEffect(() => {
    ...
    if (introPlayed === "true") {
      console.log("[LandingPage] Intro already played, skipping")
      setShowIntro(false)
      setIntroFinished(true)
    }
  }, [])

  // Idempotent onFinish handler
  const handleIntroFinish = () => {
    if (onFinishCalledRef.current) {
      console.log("[LandingPage] onFinish already called, skipping")
      return
    }
    onFinishCalledRef.current = true
    console.log("[LandingPage] Intro finished")
    setIntroFinished(true)
    setShowIntro(false)
  ...
  return (
    <>
      {/* Render IntroOverlay ONLY if showIntro === true AND introFinished === false */}
      ...
      <LandingBackground>
        ...
        <main className="min-h-screen">
          ...
          {/* Mount after intro so auth resolution does not trigger re-renders during animation */}
