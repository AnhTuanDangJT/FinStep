    ...
    return (
        <div ref={containerRef} className="force-light relative min-h-screen w-full bg-brand-bg transition-colors duration-300 overflow-hidden selection:bg-brand-primary/20 selection:text-brand-text">
            ...
            <div className="fixed inset-0 z-0">
                ...
                {/* 4. Soft Dark Grid Pattern */}
                <motion.div style={{ y: layer1Y }} className="absolute inset-0 opacity-[0.06]">
                    <div
                        className="absolute inset-0 bg-[linear-gradient(to_right,#000_1px,transparent_1px),linear-gradient(to_bottom,#000_1px,transparent_1px)] bg-[size:64px_64px] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_50%,#000_70%,transparent_100%)]"
                    />
                </motion.div>

                {/* 5. Subtle Finance Chart SVG Watermark */}
                <motion.div style={{ y: layer2Y }} className="absolute inset-0 pointer-events-none overflow-hidden flex items-center justify-center opacity-[0.04] mix-blend-multiply">
                    <svg viewBox="0 0 1000 400" className="w-[120vw] h-auto stroke-current text-black">
                        <path d="M0,400 L100,350 L200,380 L350,200 L500,250 L700,100 L850,150 L1000,50" fill="none" strokeWidth="2" />
                    </svg>
                </motion.div>

                {/* 6. Animated Glowing Dots (very low opacity) */}
                <motion.div style={{ y: layer1Y }} className="absolute inset-0 pointer-events-none">
                    <div className="absolute top-[20%] left-[15%] w-6 h-6 bg-[#ff6a00] rounded-full blur-[4px] opacity-20" style={{ animation: 'pulse-dot 4s infinite' }} />
                    <div className="absolute top-[60%] right-[20%] w-8 h-8 bg-[#ff8c00] rounded-full blur-[6px] opacity-10" style={{ animation: 'pulse-dot 6s infinite 1.5s' }} />
                    <div className="absolute top-[35%] right-[40%] w-5 h-5 bg-[#ffa500] rounded-full blur-[3px] opacity-20" style={{ animation: 'pulse-dot 5s infinite 3s' }} />
                </motion.div>
